...
import {
  Box, Typography, Chip, Button, Modal, TextField, MenuItem,
  Stepper, Step, StepLabel, StepContent, Snackbar, Alert,
  IconButton, Divider, LinearProgress, Avatar, Tooltip,
  useMediaQuery, useTheme, Fade, Slide,
} from "@mui/material";
import {
  Business, LocationOn, Apartment, ConfirmationNumber,
  MeetingRoom, Spa, FitnessCenter, Pool, LocalParking,
  Wifi, AcUnit, Close, CheckCircle, Schedule, HourglassEmpty,
  Send, ArrowForward, KeyboardArrowDown, Star, Shield,
  NotificationsActive, Person, CalendarMonth, ErrorOutline,
...
/* ─── mock data ────────────────────────────────────────────── */
const apt = {
  facility:       "Skyline Corporate Residences",
  location:       "Business Bay, Dubai, UAE",
  apartment:      "Unit 4B – Tower Alpha",
  subscriptionId: "ES-2024-XR9K2",
  roomCount:      3,
  amenities: [
...
const employee = { name: "Rayan Al-Farsi", id: "EMP-00487", dept: "Engineering" };
...
const MOCK_STATUSES = [
  { id: "REQ-0041", type: "Maintenance – HVAC", date: "12 Mar 2026", status: "In Progress", step: 2 },
  { id: "REQ-0038", type: "Housekeeping",        date: "05 Mar 2026", status: "Resolved",    step: 3 },
  { id: "REQ-0029", type: "Internet / TV Issue", date: "18 Feb 2026", status: "Pending",     step: 1 },
];
...
const statusColor = (s) =>
  s === "Resolved"    ? P.green :
  s === "In Progress" ? P.teal  :
  s === "Pending"     ? P.gold  : P.muted;

const statusIcon = (s) =>
  s === "Resolved"    ? <CheckCircle sx={{ fontSize: 14 }} /> :
  s === "In Progress" ? <HourglassEmpty sx={{ fontSize: 14 }} /> :
...
export default function ApartmentMicrosite() {
  ...
  const handleSubmit = () => {
    ...
    setSubmitted(true);
    setTimeout(() => { setRaiseOpen(false); setSubmitted(false); setReqType(""); setDesc(""); setSnack(true); }, 1800);
  };

  /* shared modal box */
  const ModalWrap = ({ children, open, onClose, wide }) => (
    <Modal open={open} onClose={onClose}
      slotProps={{ backdrop: { sx: { backdropFilter: "blur(8px)", bgcolor: "rgba(0,0,0,0.7)" } } }}>
      <Fade in={open}>
        <Box sx={{
          ...
          bgcolor: P.card, borderRadius: "12px",
  ...
  const DetailRow = ({ icon, label, value, delay = "0" }) => (
    <Box className={`fade-up delay-${delay}`} sx={{
      display: "flex", alignItems: "flex-start", gap: 2,
      p: "14px 20px", borderRadius: "8px",
      ...
      <Box>
        <Typography sx={{ fontSize: "0.62rem", letterSpacing: "0.14em", color: P.muted,
          textTransform: "uppercase", fontFamily: "'DM Sans',sans-serif", fontWeight: 600, mb: 0.3 }}>
          {label}
        </Typography>
        <Typography sx={{ fontSize: "0.95rem", color: P.text,
  ...
  return (
    <Box sx={{
      ...
      <Box sx={{ position: "fixed", inset: 0, pointerEvents: "none", zIndex: 0 }}>
        <Box sx={{
          position: "absolute", top: -180, left: -180, width: 520, height: 520,
          borderRadius: "50%",
      ...
      <Box sx={{
        ...
        px: { xs: 2.5, sm: 4, md: 6 }, pt: { xs: 4, sm: 5 }, pb: 0,
      }}>
        <Box className="fade-up" sx={{
          ...
          <Box className="float-icon" sx={{
            width: { xs: 52, sm: 64 }, height: { xs: 52, sm: 64 }, borderRadius: "14px", flexShrink: 0,
            background: `linear-gradient(135deg, ${P.teal}, ${P.cyan})`,
            display: "flex", alignItems: "center", justifyContent: "center",
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            <Box sx={{ position: "relative" }}>
              <Box sx={{
                width: 10, height: 10, borderRadius: "50%", bgcolor: P.green,
                "&::before": {
                  content: '""', position: "absolute", inset: -3,
                  borderRadius: "50%", border: `2px solid ${P.green}`,
                  animation: "pulse-ring 1.8s ease-out infinite",
                },
              }} />
            </Box>
            <Typography sx={{ fontSize: "0.75rem", color: P.green,
      ...
      <Box sx={{
        ...
        <Box sx={{
          bgcolor: P.card, borderRadius: "14px",
          border: `1px solid ${P.border}`,
          overflow: "hidden",
        }}>
          {/* card header */}
          <Box sx={{
            px: 3, py: 2, borderBottom: `1px solid ${P.border}`,
            display: "flex", alignItems: "center", justifyContent: "space-between",
            background: `linear-gradient(90deg, ${P.surface}, ${P.card})`,
          }}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
              <Box sx={{ width: 3, height: 20, borderRadius: 4,
                background: `linear-gradient(${P.teal}, ${P.cyan})` }} />
              <Typography sx={{ fontFamily: "'Syne',sans-serif", fontWeight: 700,
                fontSize: "1rem", color: P.text }}>
                Apartment Details
              </Typography>
            </Box>
            <Chip label="Verified" size="small" icon={<Star sx={{ fontSize: "12px !important", color: `${P.gold} !important` }} />}
              sx={{ bgcolor: `${P.gold}18`, color: P.gold, border: `1px solid ${P.gold}44`,
                fontSize: "0.65rem", fontFamily: "'DM Sans',sans-serif", height: 22 }} />
          </Box>

          {/* rows */}
          <Box sx={{ p: { xs: 2, sm: 3 }, display: "flex", flexDirection: "column", gap: 1.5 }}>
            <DetailRow icon={<Business sx={{ fontSize: 18 }} />} label="Facility"        value={apt.facility}       delay="1" />
            <DetailRow icon={<LocationOn sx={{ fontSize: 18 }} />} label="Location"      value={apt.location}       delay="2" />
            <DetailRow icon={<Apartment sx={{ fontSize: 18 }} />}  label="Apartment"     value={apt.apartment}      delay="3" />
            <DetailRow icon={<ConfirmationNumber sx={{ fontSize: 18 }} />} label="ESubscription ID" value={apt.subscriptionId} delay="4" />
            <DetailRow icon={<MeetingRoom sx={{ fontSize: 18 }} />} label="Room Count"   value={`${apt.roomCount} Rooms`} delay="5" />

            {/* amenities */}
            <Box className="fade-up delay-5" sx={{
              p: "14px 20px", borderRadius: "8px",
              background: `linear-gradient(135deg, ${P.surface}99, ${P.card}99)`,
              border: `1px solid ${P.border}66`,
            }}>
              <Box sx={{ display: "flex", alignItems: "center", gap: 2, mb: 1.5 }}>
                <Box sx={{
                  ...
                  display: "flex", alignItems: "center", justifyContent: "center",
                  color: P.gold,
                }}><Spa sx={{ fontSize: 18 }} /></Box>
                <Typography sx={{ fontSize: "0.62rem", letterSpacing: "0.14em",
                  color: P.muted, textTransform: "uppercase",
                  fontFamily: "'DM Sans',sans-serif", fontWeight: 600 }}>
                  Amenities
                </Typography>
              </Box>
              <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1, pl: "54px" }}>
                {apt.amenities.map(({ label, icon }) => (
                  <Chip key={label} label={label} icon={<Box sx={{ color: `${P.teal} !important`, display:"flex" }}>{icon}</Box>}
                    size="small"
                    sx={{
                      bgcolor: `${P.teal}14`, color: P.text,
                      border: `1px solid ${P.teal}33`, borderRadius: "6px",
                      fontFamily: "'DM Sans',sans-serif", fontSize: "0.75rem", fontWeight: 500,
                      height: 28,
                      "& .MuiChip-icon": { ml: "8px" },
                      "&:hover": { bgcolor: `${P.teal}25`, borderColor: P.teal },
                      transition: "all .2s",
                    }} />
                ))}
              </Box>
          ...
          {/* ── action buttons ── */}
          <Box sx={{
            px: { xs: 2, sm: 3 }, pb: 3, pt: 1,
            display: "flex", gap: 2, flexDirection: { xs: "column", sm: "row" },
          }}>
            <Button fullWidth onClick={() => setRaiseOpen(true)}
              startIcon={<NotificationsActive />}
              sx={{
                background: `linear-gradient(135deg, ${P.teal}, ${P.cyan})`,
                color: P.bg, fontFamily: "'Syne',sans-serif", fontWeight: 700,
                fontSize: "0.88rem", textTransform: "none", borderRadius: "10px",
                py: 1.5, letterSpacing: "0.02em",
                boxShadow: `0 8px 28px ${P.teal}44`,
                "&:hover": { boxShadow: `0 12px 36px ${P.teal}66`, transform: "translateY(-2px)" },
        ...
        <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
          ...
          <Box sx={{
            bgcolor: P.card, borderRadius: "14px",
            border: `1px solid ${P.border}`, p: 3,
          }}>
            <Typography sx={{ fontFamily: "'Syne',sans-serif", fontWeight: 700,
              fontSize: "0.9rem", color: P.text, mb: 2,
              display: "flex", alignItems: "center", gap: 1 }}>
              <Box sx={{ width: 3, height: 16, borderRadius: 4,
            ...
            <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 1.5, mb: 2 }}>
              {[
                ...
                <Box key={label} sx={{
                  ...
                  <Typography sx={{ fontFamily: "'Syne',sans-serif", fontWeight: 800,
                    fontSize: "1.5rem", color, lineHeight: 1 }}>{val}</Typography>
                  <Typography sx={{ fontSize: "0.65rem", color: P.muted, mt: 0.3,
            ...
            <Button fullWidth onClick={() => setStatusOpen(true)}
              ...
              sx={{
                color: P.teal, fontFamily: "'DM Sans',sans-serif", textTransform: "none",
                fontSize: "0.78rem", fontWeight: 600, borderRadius: "8px",
                border: `1px solid ${P.teal}33`, py: 0.8,
          ...
          <Box sx={{
            bgcolor: P.card, borderRadius: "14px",
            border: `1px solid ${P.border}`, p: 3, flex: 1,
          }}>
            <Typography sx={{ fontFamily: "'Syne',sans-serif", fontWeight: 700,
              fontSize: "0.9rem", color: P.text, mb: 2,
              display: "flex", alignItems: "center", gap: 1 }}>
            ...
            <Box sx={{ display: "flex", flexDirection: "column", gap: 1.5 }}>
              {MOCK_STATUSES.slice(0, 2).map((r) => (
                <Box key={r.id} sx={{
                  ...
                  background: `${P.surface}88`, border: `1px solid ${P.border}66`,
                  ...
                  <Box sx={{ flex: 1, minWidth: 0 }}>
                    <Typography sx={{ fontSize: "0.78rem", color: P.text, fontWeight: 600,
                      fontFamily: "'DM Sans',sans-serif", whiteSpace: "nowrap",
                      overflow: "hidden", textOverflow: "ellipsis" }}>
                      {r.type}
                    </Typography>
                    <Typography sx={{ fontSize: "0.65rem", color: P.muted }}>{r.date}</Typography>
                  </Box>
                  <Chip label={r.status} size="small"
                    sx={{ bgcolor: `${statusColor(r.status)}18`, color: statusColor(r.status),
                      border: `1px solid ${statusColor(r.status)}44`,
                      fontSize: "0.6rem", height: 20, fontFamily: "'DM Sans',sans-serif" }} />
                </Box>
              ))}
            </Box>
          </Box>
          ...
          <Box sx={{
            ...
            <Box sx={{ position: "absolute", right: -20, bottom: -20,
              ...
              border: `1px solid ${P.teal}22` }} />
            <Typography sx={{ fontFamily: "'Syne',sans-serif", fontWeight: 700,
              fontSize: "0.9rem", color: P.text, mb: 0.5 }}>
              Need Help?
            </Typography>
            <Typography sx={{ fontSize: "0.75rem", color: P.sub, mb: 1.5,
              fontFamily: "'DM Sans',sans-serif", lineHeight: 1.5 }}>
              Contact Accommodation Desk for urgent issues.
            </Typography>
      ...
      {/* ── footer ── */}
      <Box sx={{
        position: "relative", zIndex: 5, mt: 2,
        borderTop: `1px solid ${P.border}44`,
        px: { xs: 2.5, sm: 4, md: 6 }, py: 2.5,
        display: "flex", alignItems: "center",
        justifyContent: "space-between", flexWrap: "wrap", gap: 1,
      }}>
        <Typography sx={{ fontSize: "0.7rem", color: P.muted,
          fontFamily: "'DM Sans',sans-serif" }}>
          © 2026 StaffPortal — Accommodation Hub. All rights reserved.
        </Typography>
        <Typography sx={{ fontSize: "0.7rem", color: P.muted,
          fontFamily: "'DM Sans',sans-serif" }}>
          Last synced: March 18, 2026
        </Typography>
      </Box>
      ...
      <ModalWrap open={raiseOpen} onClose={() => setRaiseOpen(false)}>
        {/* header */}
        <Box sx={{
          px: 3, py: 2.5, borderBottom: `1px solid ${P.border}`,
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
            ...
            <Box>
              <Typography sx={{ fontFamily: "'Syne',sans-serif", fontWeight: 700,
                fontSize: "1rem", color: P.text }}>Raise a Request</Typography>
              <Typography sx={{ fontSize: "0.65rem", color: P.muted }}>
                Apartment {apt.apartment}
              </Typography>
            </Box>
          </Box>
          <IconButton onClick={() => setRaiseOpen(false)}
        ...
        {!submitted ? (
          <Box sx={{ p: 3, display: "flex", flexDirection: "column", gap: 2.5 }}>
            {/* type */}
            <TextField select fullWidth label="Request Type" value={reqType}
              onChange={(e) => setReqType(e.target.value)}
              size="small"
              sx={fieldSx}>
              {REQUEST_TYPES.map((t) => (
                <MenuItem key={t} value={t} sx={{ fontFamily: "'DM Sans',sans-serif",
                  fontSize: "0.85rem", color: P.text, bgcolor: P.card,
                  "&:hover": { bgcolor: `${P.teal}18` } }}>{t}</MenuItem>
              ))}
            </TextField>

            {/* priority */}
            <Box>
              <Typography sx={{ fontSize: "0.72rem", color: P.muted, mb: 1,
              ...
              <Box sx={{ display: "flex", gap: 1 }}>
                {["Low", "Normal", "High", "Urgent"].map((p) => (
                  <Box key={p} onClick={() => setPriority(p)}
                    sx={{
                      flex: 1, py: 0.9, borderRadius: "8px", textAlign: "center",
            ...
            {/* info row */}
            <Box sx={{ p: 1.5, borderRadius: "8px", bgcolor: `${P.gold}10`,
              border: `1px solid ${P.gold}33`, display: "flex", gap: 1.5, alignItems: "flex-start" }}>
              <ErrorOutline sx={{ fontSize: 16, color: P.gold, mt: "1px" }} />
              <Typography sx={{ fontSize: "0.72rem", color: P.sub,
                fontFamily: "'DM Sans',sans-serif", lineHeight: 1.5 }}>
                Requests are processed within 24 hours. Urgent requests are escalated immediately.
              </Typography>
            </Box>
            ...
            <Button fullWidth onClick={handleSubmit}
              ...
              sx={{
                background: (!reqType || !desc)
                  ? `${P.border}` : `linear-gradient(135deg, ${P.teal}, ${P.cyan})`,
                color: (!reqType || !desc) ? P.muted : P.bg,
                fontFamily: "'Syne',sans-serif", fontWeight: 700,
                fontSize: "0.9rem", textTransform: "none",
                borderRadius: "10px", py: 1.5,
                boxShadow: (!reqType || !desc) ? "none" : `0 8px 24px ${P.teal}44`,
                transition: "all .25s",
              }}>
              Submit Request
            </Button>
          ...
          <Box sx={{ p: 4, textAlign: "center" }}>
            <Box sx={{
              ...
              background: `${P.green}22`, border: `2px solid ${P.green}`,
              display: "flex", alignItems: "center", justifyContent: "center",
            }}>
              <CheckCircle sx={{ fontSize: 36, color: P.green }} />
            </Box>
            <Typography sx={{ fontFamily: "'Syne',sans-serif", fontWeight: 800,
              fontSize: "1.2rem", color: P.text, mb: 1 }}>
              Request Submitted!
            </Typography>
            <Typography sx={{ fontSize: "0.82rem", color: P.sub,
              fontFamily: "'DM Sans',sans-serif" }}>
      ...
      <ModalWrap open={statusOpen} onClose={() => setStatusOpen(false)} wide>
        ...
        <Box sx={{
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
            ...
            <Box>
              <Typography sx={{ fontFamily: "'Syne',sans-serif", fontWeight: 700,
                fontSize: "1rem", color: P.text }}>Request Status</Typography>
              <Typography sx={{ fontSize: "0.65rem", color: P.muted }}>
                {MOCK_STATUSES.length} requests found
              </Typography>
            </Box>
          </Box>
          <IconButton onClick={() => setStatusOpen(false)}
            sx={{ color: P.muted, "&:hover": { color: P.text } }}>
            <Close sx={{ fontSize: 18 }} />
          </IconButton>
        </Box>
      ...
      <Snackbar open={snack} autoHideDuration={4000} onClose={() => setSnack(false)}
        ...
        <Alert severity="success" onClose={() => setSnack(false)}
          sx={{ bgcolor: `${P.green}22`, color: P.text, border: `1px solid ${P.green}44`,
            fontFamily: "'DM Sans',sans-serif",
...
const fieldSx = {
  "& .MuiInputBase-root": {
    bgcolor: `#060D1A`, borderRadius: "8px",
    fontFamily: "'DM Sans',sans-serif", fontSize: "0.88rem", color: "#E2EAF4",
  },
  ...
  "& .MuiInputLabel-root": { color: "#64748B", fontFamily: "'DM Sans',sans-serif", fontSize: "0.85rem" },
